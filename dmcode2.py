				...
				if itemSold[0] == "BillNo":
					...
					if itemSold[5] == "1":		#TransactionID == 1 means a new bill
						...
						for i in range(int(itemSold[2])):
							t.appendItem(self.itemNumberToName[itemSold[1]], int(itemSold[6]))
						...
						for i in range(int(itemSold[2])):
							self.transactionList[len(self.transactionList) - 1].appendItem(self.itemNumberToName[itemSold[1]], int(itemSold[6]))
...
class Writer:
	def __init__(self, r):
		...
		with open("q2Solution.csv", "wb") as g:
